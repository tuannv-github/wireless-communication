...
% Subplot 1: Original sawtooth signal
subplot(3,1,1);
plot(t, x);
title('Sawtooth Signal');
xlabel('Time (s)');
ylabel('Amplitude');
xlim([0 0.0001]); % Zoom to 1 ms for clarity
grid on;
...
xlabel('Frequency (Hz)');
ylabel('Magnitude');
xlim([-1.5e6 1.5e6]); % Show up to 1.5 MHz
legend;
grid on;
...
title('Time Domain: Original vs Filtered Sawtooth Signal');
xlabel('Time (s)');
ylabel('Amplitude');
xlim([0 0.00001]); % Zoom to 1 ms for clarity
legend;
grid on;